...
export default function PostList({ tags }: { tags: string[] }, { store, params, derived }: WebContext) {
  const filteredPosts = derived(() => params.value?.q ? store.get<any>('posts').filter(filterSearch(params.value?.q)) : store.get<any>('posts'))
  const currentPage = derived(() => Number(params.value?.page || 1))
  const pages = derived(() => Math.ceil(filteredPosts.value?.length / itemsPerPage))
      ...
      <div class="blog-page-content">
        <div class="posts-box">
          <div class="blog-title">
            <slot name="title" />
            <div>
              {filteredPosts.value?.length} posts
            </div>
          </div>

          {postsToShow.value?.map?.(({ slug, metadata, date, timeToRead }: Post) => (
            <a
              href={`/blog/${slug}`}
              key={slug}
          ...
          {pages.value > 1 && <div class="paginator">
            {pageBadges({ currentPage: currentPage.value, pages: pages.value }).map((num: number, index: number) =>
              num ? (
                <a key={`page-${num}`}
...
function PostInfo({ date, timeToRead, hideAuthor }: { date: string, timeToRead: { text: string }, hideAuthor?: boolean }) {
...
function Tag({ key, label, q = '' }: {
  label: string
  key?: string,
  q: string
}) {
  const tag = label.toLowerCase()
  const pathname = typeof window === "undefined" ? "" : new URL(location.href).pathname;
  ...
  return (
    <a key={key} onClick={onTag} href={href} class={`tag ${isActive ? 'active' : ''}`}>
      {label}
    </a>
  )
}